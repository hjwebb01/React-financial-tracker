...
// The time range is half-open: [start_ts, end_ts), i.e., start_ts is inclusive
// and end_ts is exclusive. Returns sum in cents.
int32_t sum_by_category(int32_t category_index, int64_t start_ts,
                        int64_t end_ts, const int64_t *timestamps,
                        const int32_t *amounts, const int32_t *category_indices,
                        int32_t length) {
  ...
  int64_t total = 0;
  for (int32_t i = 0; i < length; i++) {
    if (category_indices[i] == category_index && timestamps[i] >= start_ts &&
        timestamps[i] < end_ts) {
      total += amounts[i];
    }
  }
...
int32_t sum_by_month(int32_t year, int32_t month, const int64_t *timestamps,
                     const int32_t *amounts, int32_t length) {
